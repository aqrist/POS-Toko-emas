        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('gold-prices.update', $goldPrice) }}" class="flex flex-col gap-4">
                        @csrf
                        @method('PUT')
                            ...
                            <div>
                                ...
                                <select id="branch_id" name="branch_id" class="mt-1 block w-full rounded-md border-gray-300">
                                    ...
                                    @foreach ($branches as $branch)
                                        <option value="{{ $branch->id }}" {{ old('branch_id', $goldPrice->branch_id) === $branch->id ? 'selected' : '' }}>
                                            {{ $branch->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div>
                            ...
                            <x-text-input id="market_price" name="market_price" type="number" step="0.01" class="mt-1 block w-full" value="{{ old('market_price', $goldPrice->market_price) }}" required />
                            <x-input-error class="mt-2" :messages="$errors->get('market_price')" />
                        </div>

                        <div>
                            <x-input-label for="effective_date" value="Tanggal Berlaku" />
                            <x-text-input id="effective_date" name="effective_date" type="date" class="mt-1 block w-full" value="{{ old('effective_date', $goldPrice->effective_date->toDateString()) }}" required />
